...
+function ($, maps) {
    ...
    LocationAutocomplete.prototype.init = function () {
        ...
        var mapWrap = this.$el.data('inputMap');

        var latitude = $(this.$el.data('input-latitude')).val();
        var longitude = $(this.$el.data('input-longitude')).val();

        var element = document.getElementById(mapWrap);
        ...
        this.autocomplete = new maps.places.Autocomplete(this.$el.get(0), autocompleteOptions);

        maps.event.addListener(this.autocomplete, 'place_changed', $.proxy(this.handlePlaceChanged, this));
        // maps.event.addListener(this.map, 'click', $.proxy(this.handlePlaceChanged(), this));

        // function (event) {
        //     var center = new maps.LatLng(event.latLng.lat(), event.latLng.lng());
    ...
    LocationAutocomplete.prototype.handlePlaceChanged = function () {
        ...
        $.each(valueMap['geometry'], function (standard, google) {
            ...
            if (google == 'lat') {
                $targetEl.val(geoLocation.lat());
            } else if (google == 'lng') {
                $targetEl.val(geoLocation.lng());
            }
        });
